...
description: >
  Turns venting into memory. When Ravi thinks aloud — a standup gripe, a post-demo debrief,
  a hallway observation — this skill catches the structural signals hiding in the informal
  stream (an eval gap, a prompt regression, a model behaving differently, a stakeholder's
  shifting stance, a cost spike) and routes each to the exact file where a future session
  will need it. Always proposes the routing and waits for confirmation; never writes
  sensitive content anywhere without asking. Use when the user is venting, debriefing
  informally, or thinking aloud about AI product work — not when asking a specific PM
  question or requesting a deliverable.
  Pairs with: feedback-triage (structured customer feedback, the formal sibling),
  interview-synthesis (structured transcripts), production-observability (when the vented
  signal is an ops incident), signal-scanner (when it's competitive intel).
imports:
  - feedback-flywheel
  - eval-framework
---
...
**The objective:** stop valuable signals from dying in conversation. The most important product intelligence arrives sideways — in gripes, debriefs, and thinking-aloud — and most PMs let it evaporate. This skill catches it, names what kind of signal it is, and files it where the next decision will find it. The test of success is traceability: six months later, entries in the rules, hypotheses, and eval sets should trace back to specific gossip moments.
...
- **Golden set** — the fixed collection of test cases the AI must keep passing; production failures get added to it so they can never silently return.
- **Regression** — something that used to work and now doesn't, usually after a prompt or model change.
- **Model drift** — a model's behavior changing over time or across versions, without anyone changing your product.
