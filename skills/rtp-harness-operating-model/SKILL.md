...
description: "How to fund, staff, run, and future-proof a harness as a multi-year program — the discipline around the machine, not the machine itself. Covers the real cost shape (front-loaded / plateau / compounding), the five cost centers budgets miss, the reliability dividend (why failure getting cheap is the whole business case), the lock-in and runtime wedges, the cost/quality/speed trilemma, the stopping rule; the five-rung maturity ladder + nine-day audit kit + stakeholder scripts; the four org models, the Harness PM role, human-in-the-harness, the build/buy boundary, open vs closed; and the longevity layer — the five permanent residents vs the dissolving ladder, the meta-skill four questions, and why the harness (not the model) is the moat. Use when: budgeting or defending a harness program, deciding open vs closed / build vs buy, naming the harness owner, placing your team on the maturity ladder, or deciding which components will still earn their keep in 18 months. Sibling skill: agent-harness (the architecture, diagnosis, and patterns of the machine). Pairs with: cost-model / token-economics (deep unit economics), adoption-launch (rollout), alignment-check (org readiness), capability-tracking (build-now vs wait-for-model), safety-as-moat, moat-finder. Triggers: 'harness cost', 'harness ROI', 'harness owner', 'harness maturity', 'open vs closed harness', 'is the harness the moat', 'harness org design', 'human in the harness'."
